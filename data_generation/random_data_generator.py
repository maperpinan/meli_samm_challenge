...
class DataGenerator:
    ...
    def generate_projects(self, num_projects: int) -> List[Dict[str, Any]]:
        ...
        for _ in range(num_projects):
            ...
            shared = random.choice([True, False])  # `shared` es asignado aleatoriamente
            project = {
                "id": project_id,
    ...
    def generate_tasks(self, num_tasks: int, projects: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for project in projects:
            user_id = str(self.fake.random_number(digits=7))  # Un solo user_id por proyecto
            for _ in range(num_tasks // len(projects)):
                ...
                # Define `completed_at` basado en `task["checked"]`
    ...
    def generate_activity(self, tasks: List[Dict[str, Any]], projects: List[Dict[str, Any]], num_events: int) -> List[Dict[str, Any]]:
        ...
        for _ in range(num_events):
            ...
            # Solo se agrega initiator_id si el proyecto o tarea es compartido
            initiator_id = str(self.fake.random_number(digits=7)) if item.get("shared", False) else None
            ...
            activity = {
                ...
                "parent_project_id": item.get("project_id") if task_type == "task" else item["id"],
